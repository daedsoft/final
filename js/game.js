...
function destroyPlayer(player) {
  player.setAttribute('src', 'img/explosion.png')
  setTimeout(() => {
    player.setAttribute('src', 'img/player-blue-1.png')
...
function updatePlayer(dt, $container) {
  ...
  if (GAME_STATE.spacePressed && GAME_STATE.playerCooldown <= 0) {
    createLaser($container, GAME_STATE.playerX + 9, GAME_STATE.playerY);
...
function updateEnemies(dt, $container) {
  const dx = Math.sin(GAME_STATE.lastTime / 1000.0) * 70;
...
function destroyEnemy($container, enemy) {
  enemy.$element.setAttribute('src', 'img/explosion.png')
  setTimeout(() => {
...
function updateEnemyLasers(dt, $container) {
  ...
  for (let i = 0; i < lasers.length; i++) {
    ...
    if (rectsIntersect(r1, r2)) {
      ...
      if (GAME_STATE.deaths >= PLAYER_DEATHS){        
